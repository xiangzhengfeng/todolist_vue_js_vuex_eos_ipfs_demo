...
Jurisdiction.prototype = {
	...
	add: function(address, value){		
        ...
        for(var i=0; i<=this._arrAddFrom.length; i++){
        	...
        	if(this._arrAddFrom[i] == from){
				this.totalWeight.set("addManager", allWeight.minus(fromWeight));
            }
      ...
      console.log("this.isFreezeContract",this.isFreezeContract());  
        ...
        for(var i=0; i<this._arrUpdateFrom.length; i++){
        	if(this._arrUpdateFrom[i] == from){
				this.totalWeight.set("updateManager", allWeight.minus(fromWeight));
            }
        }
	...
	del: function(address){
        ...
        for(var i=0; i<this._arrDelFrom.length; i++){
        	if(this._arrDelFrom[i] == from){
					//如果记录了一次  那么在这儿减去  然后再在后面加上
				this.totalWeight.set("delManager", allWeight.minus(fromWeight));
            }
        }
	...
	freezeContract: function(){
		...
		if(fromWeight.lt(new BigNumber(30))){
			throw new Error("当前账户地址权限低于30，无法进行冻结！");
		}
   ...
   freezeAddress: function(address){
       ...
       var freezeAddress = this.freeze.get("freezeAddress") || [];  
		if(timeDifference > 200){
            ...
            console.log("freezeAddress",this.freeze.get("freezeAddress"));
   ...
   isFreezeAddress: function(address){
		...
		var timeDifference;
      var n = 0;
       console.log("freezeAddress11111111111111111111111111111111111111111111111111111111111111111111",freezeAddress);
        ...
        for(var i=0; i<freezeAddress.length; i++){		    
           ...
           if(timeDifference < 200){		
               console.log("freezeAddress1111111111111111111111111111111",freezeAddress[i]);
       ...
       this.freeze.put("freezeAddress", newFreezeAddress);
				console.log("1111111122222222222333333333344444444444",this.freeze.get("freezeAddress"));
       console.log("1111111122222222222333333333344444444444",n);
       if(n == 1){
           return true
          }else{
          return false
	...
	//还有能够把当前各自删除  增加   更新的地址的时间记录下来  并且能够被查询到

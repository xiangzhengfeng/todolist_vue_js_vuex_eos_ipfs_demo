...
Jurisdiction.prototype = {
	...
	add: function(address, value){		
        ...
        for(var i=0; i<=this._arrAddFrom.length; i++){
        	if(this._arrAddFrom[i] == from && timeDifference.lt(new BigNumber(300))){
	...
	update: function(address, value){
        ...
        for(var i=0; i<this._arrUpdateFrom.length; i++){
        	if(this._arrUpdateFrom[i] == from && timeDifference.lt(new BigNumber(300))){
				throw new Error( "当前地址已经记录到和权重了，无需再次进行");
            } 
        }
	...
	del: function(address){
        ...
        for(var i=0; i<this._arrDelFrom.length; i++){
        	if(this._arrDelFrom[i] == from && timeDifference.lt(new BigNumber(300))){
				throw new Error( "当前地址已经记录到和权重了，无需再次进行");
            } 
        }
	...
	freezeContract: function(){
        ...
        for(var i=0; i<this._freezeContractFrom.length; i++){
        	if(this._freezeContractFrom[i] ==from && timeDifference.lt(new BigNumber(300))){
					throw new Error( "当前地址已经记录到和权重了，无需再次进行");
            }
        }
   ...
   freezeAddress: function(address){
        ...
        for(var i=0; i<this._freezeAddressFrom.length; i++){
        	if(this._freezeAddressFrom[i] == from && timeDifference.lt(new BigNumber(300))){
             throw new Error( "当前地址已经记录到和权重了，无需再次进行");
            }
        }         
   ...
   isFreezeAddress: function(address){
			...
			if(weight.gte(new BigNumber(10))){				
				...
				if(fromWeight.lt(new BigNumber(40))){
					if(timeDifference < 300){
						...
						arr.push([address,weight,timeDifference,totalValue,fromBalance]);				
					}else{
						fromBalance = (fromWeight.div(new BigNumber(100)).mul(balanceAll));
						arr.push([address,weight,new BigNumber(0),new BigNumber(0),fromBalance]);
					}
				}else{
					arr.push([address,weight,"所有者权限没有时间限制","所有者权限没有金额限制",balanceAll]);
				}
			}
            _managerOrder = [];		
		}		
	...
	// 差一个显示某个地址被正式冻结的倒计时   
    // freezeAddressWeight  只是计算了最近的这个   
	// 你可以总共冻结多个   但是你一段时间只能一个一个的冻结   因为权重都是一个一个算的
	// freezeAddressFrom只是计算了当前的这个地址  前面的所有地址清零了

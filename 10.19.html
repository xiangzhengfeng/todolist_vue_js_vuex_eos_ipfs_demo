...
Jurisdiction.prototype = {
    ...
    init: function(initialWeights) {
		...
		this.totalWeight.set(addManager, new BigNumber(50));
		...
		this._deleteRecords = new BigNumber(0);
		...
		var _order = this.addNotice.get(add) || [];
		_order.push(permissions);
		this.addNotice.put(add, _order);	
		
		this._news = "部署时将部署账户地址设置为OWNER权限，权重:"+value+";剩余未分配权重："+this._surplusPermissions;
		this.addNotice.set(this._addRecords, this._news);
	...
	add: function(address, value){
		...
		var nowAdd = Date.now();
		var startAdd = this.time.set(startAdd) || new BigNumber(0);
	...
	update: function(address, value){
		...
		var startUpdate = this.time.set(startUpdate) || new BigNumber(0);
		...
		if(from == address && value.lt(addressWeight)){
			...
			if(this._surplusPermissions.puls(addressWeight).minus(value).lt(new BigNumber(0))){
				throw new Error('按道理这儿是不能够生育权重小于零的，但是如果报错了，那就是BUG了，终止更新权限！');
			}
		}
		this.manager.set(address, value);	
		this._surplusPermissions = this._surplusPermissions.puls(addressWeight).minus(value);
        ...
        permissions.timeStamp = parseInt(nowAdd / 1000);
        permissions.address = address;
        permissions.weight = this.owner.get(address);		
		...
		_orderUpdate.push(_managerOrder.push(permissions));
	...
	del: function(address){
		...
		var nowDel = Date.now();
		var startDel = this.time.set(startDel) || new BigNumber(0);
		...
		var _orderDel = this.deleteNotice.get(del) || [];
		_orderDel.push(_managerOrder.push(permissions));
		this.deleteNotice.put(del, _orderDel);
		
		this.totalWeight.set(deleteManager, new BigNumber(0));
		this._arr = [];
		...
		this.deleteNotice.set(this._deleteRecords, this._news);
		this._deleteRecords = this._deleteRecords.plus(new BigNumber(1));
		return permissions;
	},
	...
	transfer: function(to, value){
		...
		var totalAddressValue = this.totalValue.get(address) || new BigNumber(0);
		var addressToContract = this.contractBalance.get(address) || new BigNumber(0);
		...
		if(fromWeight.gte(new BigNumber(40))){
			...
			if(timeDifference > 24*60*60){
				...
				if(valueDifference){
					this.contractBalance.set(address, balanceAll);
					this.time.set(address, nowTime);
					Blockchain.transfer(to, new BigNumber(value).valueUnit);
					totalAddressValue = totalAddressValue.plus(new BigNumber(value).mul(valueUnit));
					this.totalValue.set(address, totalAddressValue);
				}else{
		...
		_totalRecords.push(_transferOrder);
	...
	changeFreezeState: function(){
		...
		if(timeDifference >= 24*60*60){
			...
			throw new Error("冻结状态改变时效未过！");
		}
	},
	
	isFreeze: function(){
		var nowFreeze = Date.now();
		var startFreeze = this.time.set(startFreeze) || new BigNumber(0);
		var endFreeze =  new BigNumber(parseInt(nowFreeze / 1000));
		var timeDifference = endFreeze.minus(startFreeze);
		if(timeDifference >= 24*60*60){
	...
	queryOwner: function(){
		...
		var arr = [];
		for(var i=0; i< this._managerAmount; i++){
			arr.push(_managerOrder.address);
				this._arr.push(_managerOrder.address,_managerOrder.weight);
				arr.push(this._arr);
				this.managerOrder.set(owner, arr);	
			}
			arr = [];
			_managerOrder = [];
			this._arr = [];	
	...
	queryManager: function(){
		...
		for(var i=0; i< this._managerAmount; i++){
			_managerOrder = this.managerOrder.get(i);		
			if(_managerOrder.weight > 0 && _managerOrder.weight < 30){
				arr.push(_managerOrder.address);
				this._arr.push(_managerOrder.address,_managerOrder.weight);
				arr.push(this._arr);
				this.managerOrder.set(manager, arr);	
			}
			arr = [];
			_managerOrder = [];
			this._arr = [];			
	...
	queryDelete: function(){
		return this.deleteNotice.get(del);
	},
